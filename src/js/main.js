...
document.querySelector("button#submit").addEventListener("click", (event) => {
    ...
    //if liftsCount or floorsCount is invalid, show alert 
...
// identify the floor element based on the clicked button
function handleLiftCall(event){
    const button = event.target; // Get the clicked button element.
    const floorElement = button.closest('.floor'); // Find the closest ancestor with class 'floor'.
    if (floorElement) {
